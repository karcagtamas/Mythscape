...
class SessionRepositoryImpl : RepositoryImpl<SessionEntity>(), SessionRepository {
    ...
    override suspend fun <U> byCampaign(campaignId: Int, mapper: (SessionEntity) -> U): List<U> = dbQuery {
        SessionEntity.find { SessionsTable.campaign eq campaignId }.toList().map { mapper(it) }
    }

    override suspend fun <U> query(campaignId: Int?, showAll: Boolean, mapper: (SessionEntity) -> U): List<U> = dbQuery {
        SessionEntity.find {
